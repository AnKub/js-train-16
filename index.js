...
function filterStudentsByGrade(students, grade) {
  ...
  const stA = students.filter((student) => student.grade === grade);
  stA.forEach((student) => console.log(student.name));
}
...
function displayGroupedInfo(students) {
  ...
  for (const student of students) {
    console.group(`Subject: ${student.name}`);
    console.info(`HowLongDoesLive: ${student.age}`);
    console.info(`SuperPower: ${student.subject}`);
    console.groupEnd(`Subject: ${student.name}`);
  }
}
  ...
  if (userInput.username.length > 0) {
    if (userInput.password.length > 0) {
      if (userInput.password.length >= 8) {
        ...
        console.warn("пароль має бути довшим за 8 символів!");
      }
    } else {
      console.exception("пароль відсутній!");
    }
  } else {
    console.exception("ім'я користувача відсутнє!");
  }
}
...
function calculateTotalPrice(products) {
  ...
  console.timeEnd(timer);//  єдине трохи не розумію чому в мене не спрацьовую лічильник часу, просто назви кидає 
}
...
function countVowelsAndConsonants(word) {
  ...
  // Скидаємо лічильники
  const letters = "aeiou";
  for (const lett of word) {
    if (letters.includes(lett.toLowerCase())) console.count("Голосна");
